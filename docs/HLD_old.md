...
- `MockMovement` updates `MockGPS` after checking the hidden map for sphere-collision and verifying mission boundaries. Samples every 1 cm along the path. Has two constructors: the full constructor (used in production) takes GPS, hidden map, mission bounds, and drone radius; the GPS-only constructor (used in directional unit tests) skips collision and boundary checks.
- `MockGPS` stores and returns the exact position set by `MockMovement`. No rounding is applied; discretisation into grid cells is the algorithm's responsibility via `toGrid()`. `setPosition()` and `setHeading()` are independent — calling one does not affect the other.
- `MapsComparison` scores the output map against the hidden map by sampling world-space coordinates within the mission boundaries. Supports cross-resolution comparison by taking the GCD of both resolutions as the sampling step.
...
- `types::SimulationCompositionData` — composition file path, vector of `(SimulationConfigData, vector<MissionConfigData>)` tuples stored as `simulation_mission_groups`, vector of `DroneConfigData`, vector of `LidarConfigData`.
...
classDiagram
    ...
    class DroneControlImpl {
        ...
        -lidar_config_set_ bool
        -step_index_ size_t
        -last_scan_ optional
    }

    class MappingAlgorithmImpl {
        -known_voxels_ unordered_map
        -current_path_ vector
        -state_ ExplorationState
    ...
    class MapsComparison {
        +compare(original, targets, config) scores
    }
...
sequenceDiagram
    ...
    Note over Move: mission_bounds = mission.boundaries shrunk by 1cm epsilon on all sides
    Factory->>Alg:    create(synced_mission, lidar_config, drone_config, output_map)
    Note over Alg: synced_mission.boundaries = mission_bounds (same as MockMovement)
...
This diagram shows the complete internal flow of a single `DroneControlImpl::step()` call, including the LiDAR scan stored for the next step.
...
`MappingAlgorithmImpl` maintains an internal `known_voxels_` grid (separate from `output_map`) and uses Adaptive Sweep + BFS Frontier exploration to decide the next movement and scan orientation.
...
**Boundary synchronisation**: `SimulationRunFactoryImpl` shrinks `mission.boundaries` by 1cm on every side before passing them to `MappingAlgorithmImpl`. This prevents floating-point heading drift from causing the drone to plan movement to boundary cells that `MockMovement` would immediately reject. The same shrunk bounds are passed to `MockMovement`, so both components agree on where the navigable space ends.

**Initial position**: must be an exact multiple of `gps_resolution_cm`. Non-aligned positions cause `toGrid()` to snap to a different cell centre, making subsequent planned moves overshoot or cross mission boundaries.
