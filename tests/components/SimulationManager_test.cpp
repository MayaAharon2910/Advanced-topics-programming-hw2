...
// product of (simulation × mission) × drone × lidar, and calls the factory
...
// Minimal 1-sim × 1-mission × 1-drone × 1-lidar composition.
...
TEST(SimulationManager, GeneratesMultipleRunsForCartesianProduct) {
    ...
    ASSERT_EQ(report.runs.size(), 4U)
        << "SimulationManager must produce 1×2×2×1 = 4 runs from the Cartesian product";
}
